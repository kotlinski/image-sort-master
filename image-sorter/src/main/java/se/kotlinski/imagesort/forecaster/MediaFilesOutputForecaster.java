...
public class MediaFilesOutputForecaster {
  ...
  public Map<String, List<File>> calculateOutputDestinations(final Map<String, List<File>>
                                                                 mediaFilesInFolder) {
    Map<String, List<File>> exportFiles = new HashMap<>();

    for (List<File> files : mediaFilesInFolder.values()) {
      for (File file : files) {
        addMediaFileToMap(exportFiles, file);
      }
    }
    return exportFiles;
  }
  ...
  void addMediaFileToMap(final Map<String, List<File>> fileMap, final File file) {
    String outputDestination = mediaFileForecaster.forecastOutputDestination(file);
